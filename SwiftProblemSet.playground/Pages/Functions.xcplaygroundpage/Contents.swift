 ...
 */
func beginsWithVowel(_ value: String) -> Bool {
    if(value.isEmpty) {
        return false
    }

    let character = value.lowercased().characters[value.startIndex]
    return "aeiou".characters.contains(character)
}
 ...
 */
func funWithWords(_ string: String) -> String {
    return beginsWithVowel(string) ? string.uppercased() : string.lowercased()
}
